        ...
        switch (inChar) {
            ...
            case '3':
                if (this.read() == '~') {  // squencia es [3~
                    if (!(this.cursor >= this.buffer.length()-1) &&!(this.buffer.isEmpty()) ) { // en -1 si el string no esta vacio podemos eliminar lo que haya
                        //No puedes estar en el ultimo indice del string 
                        buffer = this.buffer.substring(0, this.cursor+1) + this.buffer.substring(this.cursor + 2);  // delete al carater de delante
                        this.cursor--; // baja una posicion
                    }            
                }
                break;
