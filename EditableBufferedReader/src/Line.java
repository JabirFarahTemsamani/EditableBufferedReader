...
public class Line  {  // cambiar el nombre a EditLine o Edit
    ...
    public String insertChar(char c){

        if(editBuffRea.cursor < str.length()-1 ){ // no vull que estigi en la última posició
            str = str.substring(0,editBuffRea.cursor+1) + (char) c + str.substring(editBuffRea.cursor+2);
        }
